# μ(sic) Observation Library {觀察規則庫}

本文件為 μ(sic) 的 **`Content Layer` {內容層}**。此處定義了系統所有的觀察規則、觸發條件與文字渲染模板。產品經理與設計師可直接在此文件修改文案與觸發邏輯，而無需變更核心代碼。

---

## 1. 探索與新穎度 (Novelty & Exploration)
...
title: "晨光的序曲"
trigger:
  peakListeningHour >= 4 && peakListeningHour <= 6
priority: high
rarity: rare
cooldown: 14d
emotion: hopeful
...
microcopy:
  ...
  loop_step_2: "你播放曲目的平均 BPM（速度）下降了 {decreasePercent}%。"
